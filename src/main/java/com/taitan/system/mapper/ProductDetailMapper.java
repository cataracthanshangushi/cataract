...
public interface ProductDetailMapper extends BaseMapper<ProductDetail> {
    ...
    @Select("select id,product_name,user_id,contact_id,cover,category,subheading,online,status from (\n" +
    ...
    @Select("select id,product_name,user_id,contact_id,cover,category,subheading,online,status from product_detail where online = #{online} and (product_name " +
    ...
    @Select("SELECT id,product_name,user_id,contact_id,cover,category,subheading,online,status FROM product_detail where online=1 and display=0 ORDER BY create_time DESC LIMIT 10")
    List<ProductDetailVO> getNewProDetail();

    @Select("SELECT id,product_name,user_id,contact_id,cover,category,subheading,online,status FROM product_detail WHERE display=#{display}")
